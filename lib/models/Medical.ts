...
module.exports = class Medical {
  id;
  vetId; // references contacts(id) for vet
  medicines;
  notes;
  nextApt;
  petId; // references pets(id)
  ...
  static async updateById(id:any, { vetId, medicines, notes,  nextApt , petId}:{ medicines:any, vetId:any, notes:any, nextApt:any, petId:any}) {
    const { rows } = await pool.query(
      'UPDATE medical_info SET vetId = $1, medicines = $2,  notes = $3, nextApt = $4, owner_id = $5, pet_id = $6, WHERE id = $8 RETURNING *',
